...
//<script type="text/javascript" src="https://cdn.jsdelivr.net/gh/distressco36/includeTest@main/byCityUSCANv06.js"></script>
//<script>
...
//<script>
	...
	window.addEventListener('load', function() {
		document.getElementById('app-wrapper').querySelector('.sg-icon-topic').addEventListener('click', function(){
			...
			backbtn.addEventListener('click', function(){document.getElementById('app-wrapper').removeChild(background);});
		...
		document.getElementById('app-wrapper').querySelector('#key-icon').addEventListener('click', function(e){
			...
			exitBtn.addEventListener('click', function(e){document.body.removeChild(e.target.parentNode);});
			...
			paragraph.innerHTML = "Need a Posting Key? <br><br> Use our secure Stripe&copy; payment link below to purchase your posting key. Your activated posting key will be emailed to you within 24 hours. <br><br>A posting key is valid for 1 year and allows unlimited content posting to any topic channel.";
	...
	function buildPostDash() {
		...
		attachmentInput.addEventListener('change', function(e){
			...
			divinput.addEventListener('input', function(e){
				divinput.style.height = divinput.scrollHeight;
			});
	...
	function buildPost(data, imageURL, titledata){
		...
		if(imageURL != ''){
		newPostMessage.innerHTML = `<blockquote class="twitter-tweet"><div class='blockquote-header'>Browse Topic<img class='blockquote-logo' id='blockquote-logo' src='${faviconlink}'></img></div><p lang='en' dir='ltr'><p>${resultstring}</p><div class='tweet-media-inner-div'><img class='tweet-media-img' id='media' src='${image}' alt='${image}'</img><div class='tweet-media-title'>${title}</div></div></blockquote><div class='share-icon-container'><i class="fas fa-share coin" id='share'></i></div><div class='post-format'><span class="metadata"><span class="time">${moment(timestamp).format('LLL')}</span></span></div>`;
			return(newPostMessage);
		}
		newPostMessage.innerHTML = `<blockquote class="twitter-tweet"><div class='blockquote-header'>Browse Topic<img class='blockquote-logo' id='blockquote-logo' src='${faviconlink}'></img></div><p lang='en' dir='ltr'><p>${resultstring}</p></blockquote><div class='share-icon-container'><i class="fas fa-share coin" id='share'></i></div><div class='post-format'><span class="metadata"><span class="time">${moment(timestamp).format('LLL')}</span></span></div>`;
		return(newPostMessage);
	}
	
	function processInputTxt(inputText){
		var textparsearray = inputText.trim().split(' '); 
		var resultstring = ''; var singleton = false;
	...
	function sendToDB(upfile, uptitle, uptext){
		...
		var dataToSend = {channel: channel, postid: postid, shareurl: shareURL, fileobject: upfile, caption: uptitlep,
			post: uptextp
		};
		wDB(dataToSend);
...
<div class='post-format'><span class="metadata"><span class="time">${moment(timestamp).format('LLL')}</span></span></div>
		...
		var vimpostData = {channelname: requestchannel.channel, user: userName, url: url, sitename: jsondata.provider_name,
						 title: jsondata.title, description: jsondata.description.trim(), timestamp: timestamp.toString(),
						 html: message_string.trim()}
		wDB(vimpostData);
	}
...
<div class='post-format'><span class="metadata"><span class="time">${moment(timestamp).format('LLL')}</span></span></div>
		...
		var ytpostData = {channelname: requestchannel.channel, user: userName, url: url, sitename: jsondata.provider_name,
						 title: jsondata.title, description: jsondata.author_name, timestamp: timestamp.toString(),
						 html: html_snippet.trim()}; 
		wDB(ytpostData);
		return;
	}
	...
	async function postUrlPreview(url){
		...
		if(metaData == "Failed"){console.log("No Media"); return;}
		console.log(metaData);
		linkGridHandle = document.getElementById('link-grid');
...
//<script>
	...
	window.addEventListener('load', function(e){
    	...
    	if((7<= hourofday) && (hourofday < 17)) {
      		...
      		document.documentElement.style.setProperty('--icon-color', '#1d9bf0');
      		document.documentElement.style.setProperty('--message-bubble', '#dcf8c6');
      		document.documentElement.style.setProperty('--metadata-color', '#333');
      		document.documentElement.style.setProperty('--message-bubble-received', 'WhiteSmoke');
      		document.documentElement.style.setProperty('--underline-color', '#d5d5c3');
      		document.documentElement.style.setProperty('--hyperlink-color', '#1d9bf0');
      		document.documentElement.style.setProperty('--start-statement', '#0f1419');
      		currentTheme = 'light';
			oppositecurrentTheme = 'dark';
      		return;
    	}
    		document.documentElement.style.setProperty('--background-color', '#1a1a1a');
    		document.documentElement.style.setProperty('--primary-color', '#1d9bf0');
    		document.getElementById('link-grid').style = "background-color:Black";
    		document.documentElement.style.setProperty('--text-color', 'White');
    		document.documentElement.style.setProperty('--icon-color', '#1d9bf0');
    		document.documentElement.style.setProperty('--message-bubble', '#3585fd');
			document.documentElement.style.setProperty('--metadata-color', 'White');
    		document.documentElement.style.setProperty('--message-bubble-received', '#333333');
    		document.documentElement.style.setProperty('--underline-color', '#5c5c3d');
    		document.documentElement.style.setProperty('--hyperlink-color', '#1d9bf0');
    		document.documentElement.style.setProperty('--start-statement', 'White');
    		currentTheme = 'dark';
